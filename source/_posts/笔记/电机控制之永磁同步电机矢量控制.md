...
式中，$\boldsymbol {\psi_s}$ 为`定子磁链`。且此式为三相绕组方程，对于面装式、插入式、内装式PMSM均适用。
式 ${\eqref {2-1}}$ 写成矩阵形式： 
...
式$\eqref {4-1}$表明，$i_q$ 可以控制电磁转矩（面装式PMSM电磁转矩全为励磁转矩），同样式$\eqref {3-14}$ 表明 $i_q$ 可以控制电磁转矩中的励磁转矩部分，故 $i_q$ 称为`转矩电流`。当控制 $\beta = 0^\circ \; (i_d = 0)$ 时，定子电流 $i_s$ 全部为转矩电流。
...
>以由电流 $i_A, i_B, i_C$ 变换来的 $i_d, i_q$ 为反馈量，可以计算最终输出的电流 $u_d ^\*, u_q ^\* $；
>而实际输入到逆变模块的电流 $i_A^\*, i_B^\*, i_C^\*$，则是通过对$u_d ^\*, u_q ^\* $ 进行逆变换而来。
