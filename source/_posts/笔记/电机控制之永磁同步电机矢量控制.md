...
矢量控制系统的基本模块如[图4-1](#图4-1)所示。若要通过控制电压 $u_A, u_B, u_C$ 输出，来控制 $i_d, i_q$，可以直接利用电压方程，将电流换算成电压输出；也可以使用反馈控制，将电流变换后得到的 $i_d,i_q$ 作为反馈，与期望值进行比较，进而对输出电压进行调节（如[图4-1](#图4-1)中的虚线框）。


<span id = "图4-1"></span>
> ![图4-1 矢量控制系统基本模块](4-01.png) 
