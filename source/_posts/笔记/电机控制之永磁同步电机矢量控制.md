...
[图1-3](#图1-3)为两极面装式PMSM物理模型，因为假设永磁体磁导率与空气相同，故气隙均匀，长度为 $g$，则转子直轴等效励磁电感与交轴等效励磁电感相等（$L_{md} = L_{mq}$）。将永磁体等效为置于转子槽内的励磁绕组，其有效匝数为相绕组的 $\sqrt{3 \over 2}$ 倍，通入等效励磁电流 $i_f$，在气隙产生正弦分布的励磁磁场，且等效永磁励磁磁链为 $\psi_f = L_{mf} i_f$，$L_{mf}$ 为等效励磁电感。
...
式中，$\boldsymbol {\psi_s}$ 为定子磁链。且此式为三相绕组方程，对于面装式、插入式、内装式PMSM均适用。
式 ${\eqref {2-1}}$ 写成矩阵形式： 
...
式$\eqref {2-9}$ 表时，转子等效励磁磁链矢量$\boldsymbol {\psi_f}$微分结果，即是 $\omega_r \boldsymbol{\psi_f}$ 逆时针旋转 $90^\circ$ 所得的矢量。因为 $j \omega_r \boldsymbol{\psi_f}$ 为转子磁场旋转产生的感应电动势，称为反电动势，则式$\eqref {2-8}$ 可以写成：
...
这样，定子电流矢量 ${\boldsymbol i_s}$ 可以分解为：

$$
{\boldsymbol i_s} = i_d + ji_q
{\tag {3-1}} {\label {3-1}}
$$
...
式$\eqref {3-11}$ 即可转化成式$\eqref {2-11}$所示的面装式PMSM的电压矢量方程。
插入式和内装式物理模型基本相同，固式$\eqref {3-11}$ 对插入式、内装式PMSM均适用。由式$\eqref {3-4}$和式$\eqref {3-11}$，可得插入式和内装式PMSM的稳态矢量图，如[图3-2](#图3-2)所示。
...
> 将$dp$ 轴系的 $d$ 轴与永磁体励磁磁链 $\psi_f$ 方向重合，或者说 $dp$ 轴系沿转子磁场方向定向，称之为磁场定向。后面的 $dq$ 轴系均于此磁场定向。
...
式$\eqref {4-1}$表明，$i_q$ 可以控制电磁转矩（面装式PMSM电磁转矩全为励磁转矩），同样式$\eqref {3-14}$ 表明 $i_q$ 可以控制电磁转矩中的励磁转矩部分，故 $i_q$ 称为转矩电流。当控制 $\beta = 0^\circ \; (i_d = 0)$ 时，定子电流 $i_s$ 全部为转矩电流。
PMSM使用永磁体作为转子，固转子励磁不可调节，但是可以调节电枢反应磁场；而直轴反应磁场与永磁体磁场方向共线，与永磁励磁可以相互消长，交轴反应磁场与永磁体磁场垂直，与永磁励磁的互相消长很小。
...
> $\sqrt{2 \over 3}$ 可以保证功率不变；而${2 \over 3}$ 可以保证合成空间矢量分别在A,B,C轴上的投影等于三相变量的瞬时值。
...
若要通过控制电压 $u_A, u_B, u_C$ 输出，来控制 $i_d, i_q$，可以直接利用电压方程，将电流换算成电压输出；也可以使用反馈控制，将电流变换后得到的 $i_d,i_q$ 作为反馈，与期望值进行比较，进而对输出电压进行调节，这里就不具体讲述了。
