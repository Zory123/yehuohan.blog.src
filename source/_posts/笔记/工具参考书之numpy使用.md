...
```python3
a = np.eye(3, dtype=float)  # 创建二维3x3的单位矩阵
a = np.eye(3, 4)            # 创建二维3x4的单位矩阵
b = np.identity(3)          # 创建3x3的单位矩阵
```

 - `fromstring, fromfile`：创建一维数组
...
![meshgrid](1.jpg)
 ...
 - `diag, diagflat, tri`：创建特殊矩阵

```python3
a = np.diag(np.arange(5))        # 创建对角矩阵
a = np.diagflat(np.arange(3), 1) # 对角线向上移1一个单位的对角矩阵
a = np.tri(3, 4)                 # 创建三角矩阵
```
